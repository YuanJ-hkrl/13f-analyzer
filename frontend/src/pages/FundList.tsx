      ...
      <div className="fund-grid">
        {funds.map((fund) => (
          ...
          >
            ...
            <div className="fund-top-positions"><span>Top 5 positions</span><ol>{fund.top_positions.map((position) => <li key={`${position.ticker}-${position.issuer_name}`}><b>{position.ticker || position.issuer_name}</b><small>{position.ticker ? position.issuer_name : ""}</small></li>)}</ol></div>
